...
def train(config):
    ...
    for epoch in range(1, FLAGS.iter+1):
        if FLAGS.ntype == "pos":
            ...
            loss_pos = config["k1"] * loss_pos1 + config["k2"] * loss_pos2
            loss_pos.backward()
            optimizer_pos.step()
            ...
            loss = config["k1"] * loss_pos1 + config["k2"] * loss_pos2 + config["k3"] * loss_norm1 + config["k4"] * loss_norm2 + config["k5"] * loss_pos3
            loss.backward()
            nn.utils.clip_grad_norm_(normnet.parameters(), FLAGS.grad_crip)
            optimizer_pos.step()
            optimizer_norm.step()

...
# para_list = para_list[-1].reshape(1, 11, 5)
# mad_list = np.zeros([1, 11]).astype(np.float)


# para_grid = np.arange(0.0, 6.0, 1.0)
# default_para = np.array([FLAGS.k1, FLAGS.k2, FLAGS.k3, FLAGS.k4, FLAGS.k5]).astype(np.float)
# para_list = np.tile(default_para, (5, 6, 1))
# para_list[0, :, 0] = para_list[1, :, 1] = para_list[2, :, 2] = para_list[3, :, 3] = para_list[4, :, 4] = para_grid
# mad_list = np.zeros([5, 6]).astype(np.float)
...
for i, para_i in enumerate(para_list):
    for j, para in enumerate(para_i):
        ...
        mad = np.round(mad, decimals=3)
        mad_list[i, j] = mad
        para_str = "-".join(map(str, (para*10).astype(np.compat.long).tolist()))
